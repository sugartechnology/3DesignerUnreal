...
namespace GLTFErdem
{
	...
	public:
		...
		TArray<MyFactory::FIndexVertexIdMap>    PositionIndexToVertexIdPerPrim;
	...
	MyFactoryImpl::MyFactoryImpl()
		: ImportUniformScale(1.f)
	{
		CornerVertexInstanceIDs.SetNum(3);
	}

	void MyFactoryImpl::FillMeshDescription(const GLTF::FMesh& Mesh, FMeshDescription* MeshDescription)
	{
		...
		{
			const GLTF::FPrimitive& Primitive = Mesh.Primitives[Index];
			...
			MyFactory::FIndexVertexIdMap& PositionIndexToVertexId = PositionIndexToVertexIdPerPrim[Index];
	...
	bool MyFactoryImpl::ImportPrimitive(const GLTF::FPrimitive& Primitive,  //
		int32                                          PrimitiveIndex,
		int32                                          NumUVs,
	...
	inline void MyFactoryImpl::CleanUp()
	{
	...
	MyFactory::MyFactory()
		: Impl(new MyFactoryImpl())
	{
	}

	MyFactory::~MyFactory() {}

	void MyFactory::FillMeshDescription(const GLTF::FMesh& Mesh, FMeshDescription* MeshDescription)
	{
		Impl->FillMeshDescription(Mesh, MeshDescription);
	}

	/*const TArray<GLTF::FLogMessage>& MyFactory::GetLogMessages() const
	{
		return Impl->GetLogMessages();
	}*/

	float MyFactory::GetUniformScale() const
	{
		return Impl->ImportUniformScale;
	}

	void MyFactory::SetUniformScale(float Scale)
	{
		Impl->ImportUniformScale = Scale;
	}

	void MyFactory::SetReserveSize(uint32 Size)
	{
		Impl->MaxReserveSize = Size;
	}

	void MyFactory::CleanUp()
	{
		Impl->CleanUp();
	}

	TArray<MyFactory::FIndexVertexIdMap>& MyFactory::GetPositionIndexToVertexIdPerPrim() const
	{
